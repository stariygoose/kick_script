...
export class TelegramBot {
  ...
  private loadAllowedUsers(): void {
    ...
    if (allowedUsersEnv) {
      ...
      this.logger.warn(
        "No ALLOWED_USERS found in environment variables. Bot will be accessible to everyone.",
      );
    }
  }
  ...
  private setupCallbacks(): void {
    ...
    this.bot.action("start_broadcast_with_slots", (ctx) => {
      ctx.answerCbQuery();
      this.startBroadcastWithSlotsProcess(ctx);
    });
  ...
  private async handleListUsers(ctx: Context): Promise<void> {
    ...
    const userList = usernames
      ...
      .join("\n");
    ctx.reply(
      `📋 Загружено пользователей (${usernames.length}):\n\n${userList}`,
    );
  }
  ...
  private async handleSendMessage(ctx: Context): Promise<void> {
    ...
    try {
      const result = await this.userManager.sendMessageFromUser(
        username,
        streamerNickname,
        message,
      );
      ...
      this.logger.error(
        `Failed to send message from ${username} via Telegram bot: ${error}`,
      );
    }
  }
  ...
  private async handleReload(ctx: Context): Promise<void> {
    ...
    try {
      ...
      if (this.userManager.getUserCount() === 0) {
        ctx.reply(
          `⚠️ Файл accounts.yml не найден или пуст. Бот продолжает работу с пустым списком пользователей.`,
        );
        this.logger.info(
          `accounts.yml not found during reload, continuing with empty user list`,
        );
      } else {
        ctx.reply(`❌ Ошибка перезагрузки: ${error}`);
        this.logger.error(
          `Failed to reload accounts via Telegram bot: ${error}`,
        );
      }
    }
  }
  ...
  private async handleStats(ctx: Context): Promise<void> {
    ...
    const estimatedTime =
      Math.ceil(userCount / (this.broadcastOptions.concurrency || 1)) *
      ((this.broadcastOptions.delayMs || 0) / 1000);
...
Пример: /setbroadcast 3 300`);
    ...
    const randomDelay = this.broadcastOptions.randomDelay;
    this.broadcastOptions = { concurrency, delayMs };
    if (randomDelay) {
      this.broadcastOptions.randomDelay = randomDelay;
    }
...
📈 Примерное время рассылки: ~${estimatedTime}с`);
    ...
    this.logger.info(
      `Broadcast settings updated: concurrency=${concurrency}, delay=${delayMs}ms`,
    );
  }
  ...
  private async handleExport(ctx: Context): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        `YML configuration exported and sent to user via Telegram bot`,
      );
    } catch (error) {
      ctx.reply(
        `❌ Ошибка экспорта YML: ${error}`,
        this.getBackToMenuKeyboard(),
      );
  ...
  private async handleExportTxt(ctx: Context): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        `TXT configuration exported and sent to user via Telegram bot`,
      );
    } catch (error) {
      ctx.reply(
        `❌ Ошибка экспорта TXT: ${error}`,
        this.getBackToMenuKeyboard(),
      );
  ...
  private async handleImportYml(ctx: Context): Promise<void> {
    ...
    if (state !== "waiting_yml_upload") {
      ctx.reply(
        '❌ Неожиданная команда. Используйте кнопку "Импорт YML с перезаписью" в меню файлов.',
        this.getBackToMenuKeyboard(),
      );
      return;
    }
    ...
    try {
      ...
      const processingMsg = await ctx.reply(
        "🔄 Обрабатываю YML файл и перезаписываю конфиг...",
      );
      ...
      this.userManager.importFromYmlAndOverwrite(
        tempPath,
...
Текущий файл: ${this.accountsFilePath}`;
  ...
  private showBroadcastSettings(ctx: Context): void {
    ...
    const keyboard = Markup.inlineKeyboard([
      ...
      [
        Markup.button.callback(
          "⚖️ Балансированно (3 потока, 500ms)",
          "set_balanced",
        ),
      ],
      [
...
Выберите пресет или используйте /setbroadcast <потоки> <задержка>:`;
  ...
  private setBroadcastPreset(
    ...
    const userCount = this.userManager.getUserCount();
    const estimatedTime =
      Math.ceil(userCount / (options.concurrency || 1)) *
...
📈 Новое время рассылки: ~${estimatedTime}с`);
    ...
    this.logger.info(
      `Broadcast preset "${presetName}" applied: concurrency=${options.concurrency}, delay=${options.delayMs}ms`,
    );
  ...
  private startBroadcastWithSlotsProcess(ctx: Context): void {
    const userId = this.getUserId(ctx);
    this.userStates.set(userId, "waiting_broadcast_with_slots_data");
    ...
    ctx.editMessageText(
      `🎰 Рассылка со слотами\n\nВведите данные в формате:\nstreamer_nickname основное_слово\n\n💡 К каждому сообщению автоматически добавится слово из списка, которое будет ротироваться для каждого получателя.\n\n🎲 Слова для ротации: ${this.slotWords.join(", ")}${streamersList}`,
    );
  }
  ...
  private async processRemoveUser(
    ctx: Context,
    username: string,
  ): Promise<void> {
    try {
  ...
  private async processAddStreamer(ctx: Context, input: string): Promise<void> {
    ...
    try {
      ...
      await this.updateAccountsFile();
      ctx.reply(
        `✅ Стример ${nickname} добавлен`,
        this.getBackToMenuKeyboard(),
      );
    } catch (error) {
      ctx.reply(
        `❌ Ошибка добавления стримера: ${error}`,
        this.getBackToMenuKeyboard(),
      );
    }
  }

  private async processRemoveStreamer(
    ctx: Context,
    nickname: string,
  ): Promise<void> {
    try {
      ...
      if (!streamer) {
        ctx.reply(
          `❌ Стример ${trimmedNickname} не найден`,
          this.getBackToMenuKeyboard(),
        );
        return;
      }
      ...
      await this.updateAccountsFile();
      ctx.reply(
        `✅ Стример ${trimmedNickname} удален`,
        this.getBackToMenuKeyboard(),
      );
    } catch (error) {
      ctx.reply(
        `❌ Ошибка удаления стримера: ${error}`,
        this.getBackToMenuKeyboard(),
      );
    }
  }
  ...
  private async processBroadcastWithSlots(
    ctx: Context,
    input: string,
  ): Promise<void> {
    const parts = input.trim().split(" ");
    if (parts.length < 2) {
      ctx.reply(
        "❌ Неверный формат. Используйте: streamer_nickname основное_слово",
      );
      return;
    }

    const streamerNickname = parts[0];
    const baseWord = parts.slice(1).join(" ");

    const broadcastId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    this.activeBroadcasts.set(broadcastId, { shouldStop: false });
    this.updateCounter = 0;
    this.logger.info(`Created broadcast with slots ${broadcastId}`);
  ...
  private async processSendAsUserData(
    ...
    if (parts.length < 3) {
      ctx.reply(
        "❌ Неверный формат. Используйте: username streamer_nickname message",
        this.getBackToMenuKeyboard(),
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ctx.reply(
          `✅ Сообщение отправлено от пользователя ${username}`,
          this.getBackToMenuKeyboard(),
        );
        this.logger.info(
          `Message sent from ${username} via Telegram bot interface`,
        );
      } else {
        ctx.reply(
          `❌ Ошибка отправки от ${username}: ${result.error}`,
          this.getBackToMenuKeyboard(),
        );
      }
    } catch (error) {
      ...
      this.logger.error(
        `Failed to send message from ${username} via Telegram bot interface: ${error}`,
      );
    }
  }
  ...
  private async processImportFile(
    ...
    try {
      ...
      this.userManager.importFromFileAndOverwriteYaml(
        trimmedPath,
        this.accountsFilePath,
      );
  ...
  private async processTextFileUpload(ctx: Context): Promise<void> {
    ...
    try {
      ...
      this.userManager.importFromTextAndOverwriteYaml(
        fileContent,
        this.accountsFilePath,
      );
  ...
  private handleListStreamers(ctx: Context): void {
    ...
    ctx.reply(
      `📋 Загружено стримеров (${streamers.length}):\n\n${streamerList}`,
      this.getBackToMenuKeyboard(),
    );
  }
  ...
  private handleStopBroadcast(ctx: Context, broadcastId: string): void {
    ...
    if (!broadcast) {
      this.logger.warn(
        `Broadcast ${broadcastId} not found in active broadcasts`,
      );
  ...
  public getBroadcastStopStatus(broadcastId: string): boolean {
    ...
    if (shouldStop) {
      this.logger.info(
        `getBroadcastStopStatus: Broadcast ${broadcastId} should stop = true`,
      );
    }
    return shouldStop;
  }
  ...
  private async updateTelegramMessage(
    ...
    try {
      ...
      if (error.message === "Update timeout") {
        this.logger.warn(
          "Telegram message update timeout, increasing throttle",
        );
        this.messageUpdateThrottle = Math.min(
          this.messageUpdateThrottle * 1.5,
          15000,
        );
        return;
      }
      ...
      if (error.code === 429) {
        ...
        this.messageUpdateThrottle = Math.max(
          this.messageUpdateThrottle,
          retryAfter * 1000,
        );
        return;
      }
